...
import com.happyhouse.challa.presentation.designsystem.component.ChallaCardBorder
...
/**
 * 인화 전의 빈 칸과 촬영된 흐린 사진을 순서 번호와 함께 표시하는 그리드.
 *
...
fun GalleryFilmSlotGrid(
    ...
    var captureBorder by remember(capturedPhotoUrl) { mutableStateOf(ChallaCardBorder.DASHED) }
    var isCaptureImageLoaded by remember(capturedPhotoUrl) { mutableStateOf(false) }
    var isCapturePositioned by remember(capturedPhotoUrl) { mutableStateOf(false) }
    val currentOnCaptureHighlightFinished by rememberUpdatedState(onCaptureHighlightFinished)
    val captureIndex =
        slots.indexOfFirst { slot ->
            capturedPhotoUrl != null &&
                (slot.state as? GalleryFilmSlotUiModel.State.Captured)?.imageUrl == capturedPhotoUrl
        }

    LaunchedEffect(capturedPhotoUrl, captureIndex, loadedPhotoCount) {
        if (capturedPhotoUrl == null || isCapturePositioned) return@LaunchedEffect
        if (captureIndex >= 0) {
            state.scrollToItem(captureIndex)
            isCapturePositioned = true
        } else if (
            slots.any { slot ->
                slot.state is GalleryFilmSlotUiModel.State.Captured && slot.state.imageUrl == null
            }
        ) {
            onLoadMore()
        }
    ...
    GalleryGridLayout(
        ...
        items(
            ...
            val isNewCapture =
                capturedPhotoUrl != null &&
                    (slotState as? GalleryFilmSlotUiModel.State.Captured)?.imageUrl == capturedPhotoUrl

            ChallaCardItem(
                border = if (isNewCapture) captureBorder else ChallaCardBorder.DEFAULT,
                onImageLoadFinished = {
                    if (isNewCapture) {
                        isCaptureImageLoaded = true
                    }
                },
                order = slot.order,
                type =
